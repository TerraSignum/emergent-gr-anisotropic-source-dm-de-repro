...
algebra against the parent corpus's independent 8-layer
cc-dressing pipeline output (bundled here as
data/cc_dressing_pipeline.json from the parent corpus's
GCC-01 module).

The 8-layer dressing pipeline computes the cosmological-constant
...
def main():
    ...
    print("=" * 78)
    print("Phase-H: Cross-validation of Path 5 against the 8-layer")
    print("cc-dressing pipeline (parent corpus GCC-01).")
    print("=" * 78)
    print()

    # 8-layer pipeline (P1 regime)
    ...
    print("8-layer cc-dressing pipeline (GCC-01, P1 regime):")
    ...
    print(f"  cc_orders_explained (incl. 2 corr)   = {cc_orders}")
    ...
    print("The 8-layer pipeline reduces this by ~123 OoM to rho_observed;")
    ...
    out = {
        ...
        "cross_check": {
            ...
            "interpretation": (
                ...
                "to a Planck-scale starting point that the 8-layer "
                "dressing pipeline reduces by ~ 123 orders of magnitude."
            ),
        },
        "consistency_verdict": (
            "Path 5 and the GCC-01 8-layer dressing pipeline are "
            ...
            "verifies HOW the IR reduction proceeds (8-layer "
            "dressing)."
        ),
    }
