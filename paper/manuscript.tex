...
\newcommand{\repoOwn}{emergent-gr-anisotropic-source-dm-de-repro}
...
\!=\!-1+\bigl[(1/400)/(1/10)\bigr]\!=\!-1+1/40\!=\!-39/40\!=\!-0.975$,
a small finite
...
System-$\mathcal R$ level. An earlier candidate
paired-degeneracy correction does not enter the published
nine-layer chain: the corresponding analysis (recorded in
the closure-data bundle) showed it to be structurally
distinct from the suppression layers used here.
...
Hubble-tension state of the art as of late
2025~\cite{DiValentino2025Tensions} consolidates the
discrepancy at $\sim\!6\sigma$ in the combined post-DESI-DR2
+ JWST landscape, with neither EDE, modified-gravity, nor
dynamical-dark-energy remedies yet closing the early-/late-
time split; the framework's structural $67.5$ value sits on
the early-universe side of this split. The $\sim\!5\sigma$
SH0ES Cepheid distance-ladder tension ($73.04\!\pm\!1.04$)
is identified by the framework as a Cepheid-systematic rather
than a framework failure; the converging trend of recent
...
\emph{Physical-reading proposal for the structural product.}
The dimensionless Hubble parameter
$h\!\equiv\!H_{0}/(100\,{\rm km/s/Mpc})$ closes algebraically as
\begin{equation}
\label{eq:h_structural_proposal}
h\;=\;\alpha_{\xi}\,s_{\rm face}\,N_{\rm gen}
   \;=\;\frac{N_{\rm gen}(2d{+}1)}{2d(d{+}1)}
   \;=\;\frac{3\!\cdot\!9}{8\!\cdot\!5}\;=\;\frac{27}{40}\;=\;0.675,
\end{equation}
where the numerator $N_{\rm gen}(2d{+}1)\!=\!27$ is the
\emph{generation-replicated carrier-axis content}
(three generations $\times$ nine carrier basis modes
counted by $2d{+}1$, the central-$\Xi$-augmented chirality-
doubled spatial dimension that anchors $\alpha_{\xi}\!=\!9/10$
in the bit-information reading
$d_{\rm eff}\!=\!\log_{2}(2d{+}1)$), and the denominator
$2d(d{+}1)\!=\!40$ is the
\emph{sync-extended chirality-doubled spatial volume}
($2d\!=\!8$ chirality-doubled spatial DoF times $d{+}1\!=\!5$
sync-cone extension). Reading $h$ as ``axis-bits per
sync-spatial DoF, with the $N_{\rm gen}$-fold generation
multiplicity'' identifies the Hubble rate with the
carrier-graph's emergent expansion rate per generation-bundle.
The unit factor $100\,{\rm km/s/Mpc}$ is not free either: it is
the dimensional content set by the standard Friedmann equation
at $z\!=\!0$, with the cosmological inputs
$(\rho_{\Lambda}, \Omega_{\Lambda})$ themselves closed
independently in the present paper. A \emph{cross-projection
identity test} is reported in
\pyown{src/verify_H0_friedmann_cross_projection.py}{verify_H0_friedmann_cross_projection} (bundled
output \pyown{outputs/verify_H0_friedmann_cross_projection.json}{verify_H0_friedmann_cross_projection.json}):
\textbf{Route A (carrier-product)}
$H_{0}\!=\!100\!\cdot\!\alpha_{\xi}s_{\rm face}N_{\rm gen}
\!=\!67.50$~km/s/Mpc; \textbf{Route B (Friedmann at $z\!=\!0$)}
$H_{0}\!=\!\sqrt{(8\pi/3)\rho_{\Lambda}/(M_{\rm Pl}^{2}
\Omega_{\Lambda})}\!=\!66.93$~km/s/Mpc, taking
$\rho_{\Lambda}\!=\!2.49\!\times\!10^{-47}$~GeV$^{4}$ from the
nine-layer dressing closure
(\S\ref{sec:cc_nine_layer_dressing}, EXACT $0.1\%$) and
$\Omega_{\Lambda}\!=\!103/150$ from
Eq.~\eqref{eq:Omega_m_Lambda}. The two routes converge to
within $0.85\%$ (PRECISE), and both lie within Planck~2018
$1\sigma$ of $H_{0}\!=\!67.4\pm 0.5$~km/s/Mpc. The $27/40$
product is therefore \emph{not} a free numerical coincidence
of three independently chosen primitives but a structural
identity required by the Friedmann equation given the
framework's independent $\rho_{\Lambda}$ and $\Omega_{\Lambda}$
closures. The simpler reading of $h$ as a single
spectral observable --- the carrier's per-Fiedler-period
expansion rate
$|d\ln\langle|\Xi|\rangle/dt|/\sqrt{\lambda_{2}}$ on the
vacuum branch, with $\lambda_{2}$ the Fiedler eigenvalue of
the normalised carrier Laplacian --- is excluded by the
in-repo audit
\path|relational-uv-closure-repro/src/verify_ET9_H0_fiedler_spectral_ratio.py|
across $96$ vacuum-branch seeds, which finds
$h_{\rm emp}\!=\!0.0013\!\pm\!0.0002$ and rejects the
identification at $-99.8\%$ relative residual to $27/40$.
The Hubble parameter is therefore not a single spectral
quantity of the carrier but the multiplicative composition
of three independently-derived primitives, with the Friedmann
equation supplying the cross-validation.
...
reading and version history.}
The framework's \emph{canonical} prediction is
$$\Sigma m_{\nu}\;=\;m_{1}\!+\!m_{2}\!+\!m_{3}\;=\;0.0593\,\mathrm{eV}\qquad
\text{(NH; canonical closure, see Eqs.~\eqref{eq:p4b_m3_nu_dressed}--\eqref{eq:p4b_m2_nu_dressed}).}$$
This composes the seesaw closure
$m_{1}\!=\!\gamma^{2}m_{3}\!=\!5.023\!\times\!10^{-4}$~eV with the
parameter-free single-vertex carrier-defect dressings
$(1\!-\!\gamma^{2}(d{+}N_{\rm gen})/4)$ on $m_{3}$ and
$(1\!+\!\gamma^{2}(d{+}N_{\rm gen}))$ on $m_{2}$ (P4B
Eqs.~\eqref{eq:p4b_m3_nu_dressed},
\eqref{eq:p4b_m2_nu_dressed}); reproducer
\pyown{src/verify_neutrino_masses_m1_m2.py}{verify_neutrino_masses_m1_m2}
returns the canonical 0.0593\,eV value.
\textbf{NH-absolute-minimum reading (m\_1 dropped).} If $m_{1}$ is
formally set to zero (below typical detection threshold), the
sum reduces to $\Sigma m_{\nu}^{\rm NH,min}\!=\!m_{2}\!+\!m_{3}\!=\!0.05883$~eV
(strict-EXACT $0.05\%$ vs the NuFIT~6.1 oscillation lower
bound); this is a \emph{subset} of the canonical reading, not a
competing closure.
\textbf{Superseded.} The earlier closure
$m_{1}\!=\!m_{3}/(d\!+\!1)$ giving
$\Sigma m_{\nu}\!=\!0.0603$~eV was falsified by DESI DR2 under
$\Lambda$CDM (the value sits at the upper edge of the data
band); the canonical 0.0593\,eV value uses the
$m_{1}\!=\!\gamma^{2}m_{3}$ closure instead. Any in-corpus
mention of 0.0603\,eV is legacy and should be read as
``superseded; see P4B canonical reading''.
\textbf{Even-older superseded.} The
$m_{1}\!=\!m_{3}/5$ closure
($\Sigma m_{\nu}\!=\!0.0732$~eV) was falsified by earlier
DESI bounds and is retained only for reference.

The DESI DR2 BAO + DR1 full-shape
analysis~\cite{DESI2025} reports
$\Sigma m_{\nu}\!<\!0.0642$~eV at $95\%$ C.L.\ under
flat-$\Lambda$CDM with three degenerate states; the
framework's canonical $0.0593$~eV value sits $7.6\%$ below
that upper edge and $\sim\!1\%$ above the NH-absolute-minimum
oscillation lower bound — a sharp falsifiable next-generation
handle. CMB-S4 / Roman / Euclid will decide within $\sim\!2$
years whether $\Sigma m_{\nu}$ lands inside or outside the
framework window. Under $w_{0}w_{a}\mathrm{CDM}$ the DESI bound
relaxes to $<\!0.163$~eV~\cite{DESI2025} — the test is
sharper under $\Lambda$CDM and weaker under dynamical
dark energy.
...
$w_{a}^{\rm DESI}\!\approx\!-0.4$ at $\sim\!3.1\sigma$
significance~\cite{DESI2025BAO,DESI2025} (DESI~DR2~BAO+CMB
combination) lies $\sim\!40\times$ outside
...
The DESI-DR2 dynamical-DE signal is itself under live debate
in the literature: an independent analysis~\cite{WangMota2025}
argues that the apparent preference for evolving $w(z)$ is
\emph{not robust} once tensions among the CMB, BAO and SNe
datasets are propagated through the combined likelihood ---
a non-detection that, if it consolidates, would relieve the
present $\sim\!40\times$ pressure on the framework band
without requiring DR3.
\end{enumerate}
...
$n_{s} = 1 - (d{+}N_{\rm gen})\,\gamma\,\varepsilon^{2}_{\text{sync}}
       = 1 - \gamma^{2}(d{+}N_{\rm gen})/2 = 193/200$
(using $\varepsilon^{2}_{\text{sync}} = \gamma/2$).
...
An independent recent line in the cosmological-structure
literature shows that a distribution of (massless) topological
defects can reproduce flat rotation curves and isothermal-
sphere-style lensing signatures without an underlying particle
component~\cite{Lieu2024}, supporting the present
identification of the halo signal with
$T_{\mu\nu}^{\text{DM-vortex}}$ rather than with particulate DM.
...
Reproducer
\pyown{src/verify_wDE_chirality_running.py}{verify_wDE_chirality_running}.
...
(\pyown{src/verify_signed_halo_R0i_and_R00_null.py}{verify_signed_halo_R0i_and_R00_null};
output \pyown{outputs/verify_signed_halo_R0i_and_R00_null.json}{verify_signed_halo_R0i_and_R00_null.json}):
...
branch-resolved $\Lambda_{t}$ identity (Paper~4~\cite{Paper4},
Sec.~``Branch-resolved
$\Lambda_{t}/T_{00}/G_{00}$'') splits this further into
...
consequence of the System-$\mathcal R$ algebra.
...
\pyown{src/verify_signed_vs_magnitude_halo_separation.py}{verify_signed_vs_magnitude_halo_separation};
output
\pyown{outputs/verify_signed_vs_magnitude_halo_separation.json}{verify_signed_vs_magnitude_halo_separation.json}).
...
answer
(\pyown{src/verify_frw_lattice_epoch_identification.py}{verify_frw_lattice_epoch_identification};
output \pyown{outputs/frw_lattice_epoch_identification.json}{frw_lattice_epoch_identification.json})
...
\input{tables/tab_R00_sign_fractions}
\end{table}
...
Reproducer
\pyown{src/verify_KQ_top_pct_chi2_sweep.py}{verify_KQ_top_pct_chi2_sweep}, output
\pyown{outputs/verify_KQ_top_pct_chi2_sweep.json}{verify_KQ_top_pct_chi2_sweep.json}; the
...
reproducer
\pyown{src/verify_strong_lensing_refinements.py}{verify_strong_lensing_refinements} O4)
...
\eta_{b} \;&=\; (d + N_{\mathrm{gen}})^{2}\,\gamma^{10} / (2 d)
         \;=\; 49/(8\!\cdot\!10^{10})
         \;=\; 6.125\!\times\!10^{-10}
         \quad \text{PRECISE}\,0.41\%\,(z\!=\!+0.42
         \nonumber \\
         & \qquad\quad \text{vs Planck 2018 } (6.10\!\pm\!0.06)\!\times\!10^{-10}),
...
beyond $4\sigma$ each.
\item[(H-T-3)] \emph{Two-camp split of 2024-2026
distance-ladder analyses.}
The SH0ES Cepheid analysis with JWST data
(Riess~2024~\cite{Riess2024JWST}, three-method
JWST result $H_{0}\!=\!72.6\!\pm\!2.0$~km/s/Mpc on the
distance-limited $D\!\leq\!25$~Mpc sample) \emph{maintains} the
$\sim\!73$~km/s/Mpc value with HST--JWST mutual consistency at
the $\sim\!1\sigma$ level, attributing previous CCHP-vs-SH0ES
discrepancies to supernova-subsample selection rather than
Cepheid-calibration systematics. The CCHP analysis
(Freedman~2024~\cite{Freedman2024CCHP}) with JWST TRGB at
$H_{0}\!=\!68.81\!\pm\!2.22$~km/s/Mpc and JAGB at
$67.80\!\pm\!2.72$~km/s/Mpc remains compatible with the
framework's $67.5$~km/s/Mpc prediction. The two camps therefore
present a persistent $\sim\!2\sigma$-level subsample-attribution
debate within the JWST-anchored local-ladder sample; the
framework's structural prediction is compatible with the
non-Cepheid (TRGB/JAGB) JWST channels and the
Planck~\cite{Planck2018}/DESI~\cite{DESI2025BAO} CMB-anchored
late-time values, and at tension with the Cepheid-anchored
SH0ES channel. The persistence of the Cepheid--TRGB/JAGB split
under JWST validation is itself the principal load-bearing
evidence for the framework's attribution of the SH0ES tension
to the Cepheid step.
\end{enumerate}
...
across nine cross-sector observables (\pyown{data/closure_derivations/HW_indirect_audit.json}{HW_indirect_audit.json}) finds $\Delta\chi^{2}\!=\!+1.15$
...
the $\Lambda$CDM prior. The alternative
...
$\chi$-like metric (cf.~\pyown{data/closure_derivations/HAA_d_Ngen_derivation.json}{HAA_d_Ngen_derivation.json}):
\begin{equation}
...
\quad (1.64\% \text{, PRECISE; alternative}\nonumber\\
&\quad \text{combinatorial reading. Canonical } \eta_{B}\text{ closure
is the}\nonumber\\
&\quad (d{+}N_{\rm gen})^{2}\gamma^{10}/(2 d)
\text{ form of Eq.~\eqref{eq:etab_closure})}
...
m_{3}^{(\nu)} &= m_{e}\,\gamma^{d+3}\!\left(1\!-\!\tfrac{\gamma^{2}(d{+}N_{\rm gen})}{4}\right)
            = 0.05023\,\text{eV} \nonumber \\[2pt]
&\quad \text{vs } \sqrt{\Delta m_{31}^{2}} = 0.0502\,\text{eV}
\quad (0.068\% \text{, EXACT; NuFIT 6.1; tree } m_e\,\gamma^7 = 0.0511 \text{ eV})
\label{eq:p4b_m3_nu} \\[6pt]
m_{2}^{(\nu)} &= m_{e}\,\gamma^{d+4}\,\tfrac{\pi}{2}\!\left(1\!+\!\gamma^{2}(d{+}N_{\rm gen})\right)
            = 0.008594\,\text{eV} \nonumber \\[2pt]
&\quad \text{vs } \sqrt{\Delta m_{21}^{2}} = 0.00859\,\text{eV}
\quad (0.041\% \text{, EXACT; NuFIT 6.1})
\label{eq:p4b_m2_nu} \\[6pt]
\Sigma m_{\nu}^{\rm NH,min} &= m_{2}^{(\nu)}\!+\!m_{3}^{(\nu)} = 0.05883\,\text{eV} \nonumber \\[2pt]
&\quad \text{vs NuFIT 6.1 NH-min} \quad (0.05\% \text{, EXACT; }< 0.0642\,\text{eV DESI DR2})
\label{eq:p4b_Sigma_mnu} \\[6pt]
Y_{p} &= \frac{1}{d+1} + \varepsilon_{\rm sync}^{2}
       - \alpha_{\rm EM} = 0.243 \nonumber \\[2pt]
&\quad \text{vs PDG 2024 } 0.245
\quad (0.93\% \text{, PRECISE; alternative BBN-freeze-out}\nonumber\\
&\quad \text{decomposition; canonical }Y_{p}\text{ closure is the}\nonumber\\
&\quad (d{+}N_{\rm gen})^{2}\gamma^{2}/2\!=\!49/200\text{ form of
Eq.~\eqref{eq:Yp_closure}}\nonumber\\
&\quad \text{at strict-EXACT } 0.000\%)
\label{eq:p4b_Yp}
\end{align}
...
top-$0.5\%$ $t_{00}(a)$; per
\pyown{src/verify_chirality_local_RG_window.py}{verify_chirality_local_RG_window} on the canonical
...
\pyown{src/verify_KQ_top_pct_chi2_sweep.py}{verify_KQ_top_pct_chi2_sweep}), which pins the
...
and reproducer
\pyown{src/verify_signed_halo_component_decomposition.py}{verify_signed_halo_component_decomposition}, with
...
The tree-level form $m_{3}^{(\nu),\rm tree}\!=\!m_{e}\,\gamma^{d+3}\!=\!m_{e}\,\gamma^{7}$
connects the electron mass to the atmospheric-neutrino mass via
chirality-sine to power $d\!+\!3\!=\!7$. Numerically
$5.1129\!\times\!10^{5}\!\cdot\!10^{-7}\!=\!0.05113$~eV vs
$\sqrt{\Delta m_{31}^{2}}\!=\!0.0502$~eV at $1.85\%$ residual.
The parameter-free single-vertex carrier-defect dressing
$(1\!-\!\gamma^{2}(d{+}N_{\rm gen})/4)\!=\!(1\!-\!7\gamma^{2}/4)$
promotes the closure to
\begin{equation}
m_{3}^{(\nu)}
   \;=\; m_{e}\,\gamma^{d+3}\!\left(1\!-\!\tfrac{\gamma^{2}(d{+}N_{\rm gen})}{4}\right)
   \;=\; 0.05023~\mathrm{eV},
\label{eq:p4b_m3_nu_dressed}
\end{equation}
strict-EXACT at $0.068\%$ vs $\sqrt{\Delta m_{31}^{2}}\!=\!0.0502$~eV.
The complementary solar-neutrino tree form
$m_{2}^{(\nu),\rm tree}\!=\!m_{e}\,\gamma^{d+4}\,(\pi/2)\!=\!0.00803$~eV
admits the dual parameter-free dressing
\begin{equation}
m_{2}^{(\nu)}
   \;=\; m_{e}\,\gamma^{d+4}\,\tfrac{\pi}{2}\!\left(1\!+\!\gamma^{2}(d{+}N_{\rm gen})\right)
   \;=\; 0.008594~\mathrm{eV},
\label{eq:p4b_m2_nu_dressed}
\end{equation}
strict-EXACT at $0.041\%$ vs
$\sqrt{\Delta m_{21}^{2}}\!=\!0.00859$~eV.

\paragraph{Universal $\gamma^{2}(d{+}N_{\rm gen})$ carrier-defect
leakage: a cross-sector correspondence.}
The same single structural integer $d\!+\!N_{\rm gen}\!=\!7$ ---
the sum of independent geometric quantum numbers (spacetime
dimension plus generation count) --- controls the $\gamma^{2}$-order
single-defect-channel leakage in five a priori unrelated
sectors:
\begin{equation}
\begin{aligned}
n_{s}                 &\;=\; 1 - \tfrac{\gamma^{2}(d{+}N_{\rm gen})}{2}
                      \;=\; \tfrac{193}{200}
                      & \text{(CMB scalar tilt; }N_{\rm chir}/d\text{ coefficient)},\\
m_{3}/m_{3}^{\rm tree} &\;=\; 1 - \tfrac{\gamma^{2}(d{+}N_{\rm gen})}{4}
                      & \text{(atmospheric }\nu\text{-mass; }1/d\text{)},\\
m_{2}/m_{2}^{\rm tree} &\;=\; 1 + \gamma^{2}(d{+}N_{\rm gen})
                      & \text{(solar }\nu\text{-mass; full)},\\
\frac{m_{b}/m_{s}}{(m_{b}/m_{s})^{\rm tree}}
                      &\;=\; 1 - \tfrac{\gamma^{2}(d{+}N_{\rm gen})}{4}
                      & \text{(heavy-light quark ratio; }1/d\text{)},\\
(D/H)/(D/H)^{\rm tree}
                      &\;=\; 1 + \tfrac{\gamma^{2}(d{+}N_{\rm gen})}{4}
                      & \text{(primordial }D/H\text{; }1/d\text{; BBN}).
\end{aligned}
\label{eq:dNgen_universal_leakage}
\end{equation}
The coefficient hierarchy $1\!:\!1/d\!:\!N_{\rm chir}/d$
($=\!1\!:\!1/4\!:\!1/2$) reflects, in order, the full
single-vertex coupling for the lightest neutrino mode (solar),
the spacetime-dimension suppression for one-point residues with
single $d$-direction projection (atmospheric $\nu$, heavy-light
quark ratio, $D/H$), and the chirality-pair averaging
$N_{\rm chir}/d$ for the scalar two-point correlator (CMB tilt).
Five a priori-disjoint sectors spanning $\sim\!50$ orders of
magnitude in energy --- from inflation at $10^{16}$ GeV through
quark masses at GeV to neutrino seesaw at $10^{-2}$ eV --- thus
close on the \emph{same} carrier-defect leakage scale
$\gamma^{2}(d{+}N_{\rm gen})$, parameter-free under the
framework's first-principles rationals.

\emph{Distinct from the universal leakage: the
$(d{+}N_{\rm gen})^{2}$ BBN/reionization prefactor family.}
Four BBN/reionization observables share a SEPARATE structural
unification through the squared dimensional sum
$(d{+}N_{\rm gen})^{2}\!=\!49$ as a clean integer prefactor:
$Y_{p}\!=\!(d{+}N_{\rm gen})^{2}\gamma^{2}/2\!=\!49/200$
(eq.~\eqref{eq:Yp_closure}),
$\eta_{B}\!=\!(d{+}N_{\rm gen})^{2}\gamma^{10}/(2 d)\!=\!49/(8\!\cdot\!10^{10})$
(eq.~\eqref{eq:etab_closure}),
$\tau_{\rm re}$, and $z_{\rm re}$
(eqs.~\eqref{eq:taure_closure},~\eqref{eq:zre_closure}). The
canonical $\eta_{B}$ closure thus belongs to this
$(d{+}N_{\rm gen})^{2}$ prefactor family, \emph{not} to the
universal $\gamma^{2}(d{+}N_{\rm gen})/d$ leakage family above ---
two distinct structural unifications coexist, each parametrising
different physical channels. The 7/24 cross-check
$\eta_{B}/A_{s}\!=\!(d{+}N_{\rm gen})/(2 d N_{\rm gen})\!=\!7/24$
matching PDG at $0.04\%$ rel-err is itself the same $7/24$
integer ratio as the $(SG)$-axiom Newman-clustered skeleton-gap
target (P4 prop.\ Statistical-fingerprint), suggesting a deeper
connection between the BBN $(d{+}N_{\rm gen})^{2}$ family and
the carrier-graph spectral structure.

\textbf{Eq.~\eqref{eq:p4b_Sigma_mnu} sum of neutrino masses.}
The NH-minimum sum
$\Sigma m_{\nu}\!=\!m_{1}\!+\!m_{2}\!+\!m_{3}\!\to\!m_{2}\!+\!m_{3}$
(at $m_{1}\!\to\!0$) combines the dressed structural forms
\eqref{eq:p4b_m3_nu_dressed} and \eqref{eq:p4b_m2_nu_dressed} to
give $0.008594\!+\!0.05023\!=\!0.05883$~eV, matching the NuFIT~6.1
NH-minimum oscillation lower bound at $0.05\%$ strict-EXACT, and
consistent with the DESI DR2 + DR1 full-shape
...
(PRECISE). The \emph{canonical} $Y_{p}$ closure of
Eq.~\eqref{eq:Yp_closure} is the
$(d{+}N_{\rm gen})^{2}\gamma^{2}/2\!=\!49/200\!=\!0.24500$
form at strict-EXACT $0.000\%$, part of the unified
$(d{+}N_{\rm gen})^{2}$ BBN/reionization prefactor family.

Reproducers:
\pyown{src/verify_neutrino_mass_DESI_2024.py}{verify_neutrino_mass_DESI_2024},
\pyown{src/verify_post_flip_dark_sector_theories.py}{verify_post_flip_dark_sector_theories},
\pyown{src/verify_eta_B_precise_form.py}{verify_eta_B_precise_form},
...
(reproducer
\pyscript{emergent-gr-schwarzschild-ppn-repro}{src/verify_smacs_j0723_multicomponent.py}{verify_smacs_j0723_multicomponent}).
Reproducer for the eight-system table:
\pyscript{emergent-gr-schwarzschild-ppn-repro}{src/verify_strong_lensing_extended.py}{verify_strong_lensing_extended}.
...
Reproducer: \pyown{src/verify_sparc_real_175.py}{verify_sparc_real_175} with
...
(\pyown{src/verify_sparc_per_galaxy_patterns.py}{verify_sparc_per_galaxy_patterns}) shows that
...
(reproducer
\pyown{src/verify_sparc_two_phase_statistical_hardening.py}{verify_sparc_two_phase_statistical_hardening},
bundle
\pyown{outputs/verify_sparc_two_phase_statistical_hardening.json}{verify_sparc_two_phase_statistical_hardening.json})
gives:
...
$V_{\rm flat}$-only test (\pyown{src/verify_sparc_two_phase_carrier.py}{verify_sparc_two_phase_carrier})
...
\pyown{src/verify_sparc_two_phase_refined.py}{verify_sparc_two_phase_refined}; bundle
\pyown{outputs/verify_sparc_two_phase_refined.json}{verify_sparc_two_phase_refined.json};
hardening:
\pyown{src/verify_sparc_two_phase_statistical_hardening.py}{verify_sparc_two_phase_statistical_hardening}.
...
mass-bias prediction (\pyown{src/verify_cluster_xray_vs_lensing.py}{verify_cluster_xray_vs_lensing})
...
(\pyown{src/verify_cluster_xray_vs_lensing.py}{verify_cluster_xray_vs_lensing}): Coma
...
\pyown{src/verify_continuum_bianchi.py}{verify_continuum_bianchi} reanchoring the
bundled lattice audit
\pyown{outputs/discrete_bianchi_scaling_audit.json}{discrete_bianchi_scaling_audit.json}.
...
\footnotesize
\input{tables/tab_dense_cell_frustration}
\normalsize
\end{table}
...
\begin{table}[!htbp]
\centering\footnotesize
\input{tables/tab_R00_shape_fit_aicc.tex}
...
\paragraph{Candidate paired-degeneracy correction not used.}
An earlier candidate ninth layer (a paired-degeneracy
correction) does not enter the published nine-layer chain.
The reasoning is reproduced here from the closure-data bundle
(\pyown{data/cosmological_constant_closure.json}{cosmological_constant_closure.json}): (i) the
eight upstream layers (six additive suppression layers plus
zero-mode cancellation and backreaction screening) already
sum to $-122.99$ OoM, exceeding the required $122.95$ OoM by
$0.05$ OoM, so the $122$-order hierarchy is fully accounted
for without the paired-degeneracy correction; (ii) the
candidate was sign-inconsistent across the code path (the
production residual-closure script applied it as
un-suppression $+0.32$ OoM, while the
System-$\mathcal R$ identification asserted suppression
$-0.33$ OoM); (iii) the layer admits no external
quantum-field-theory analogue (own framing: ``internal to
the lattice construction''); (iv) either sign would overshoot
the closure further than the eight-layer chain alone (ratio
$1.83$ with $+0.32$, ratio $0.42$ with $-0.33$, vs.\ ratio
$0.89$ with the layer dropped). The published nine-layer chain
replaces this candidate with the derived synchronization-
channel un-cancellation $H_{\rm sync}$ (P2~\S5 $C_{3}$),
closing the residual to $+0.0004$ OoM (EXACT) with the
canonical-regime residual at $-0.05$ OoM.
...
\pyown{src/verify_proxy_kRec_bianchi_falsification.py}{verify_proxy_kRec_bianchi_falsification}.
...
nine-layer cosmological-constant dressing chain. \emph{All nine
sub-layers are derived} --- the chain has $0$ free-parameter
fit layers. The derivations fall in three classes.
\emph{Seven layers carry closed-form identifications} under the
master-derivation primitives
$(d, N_{\mathrm{gen}}, \gamma, \alpha_{\xi}, \varepsilon_{\rm sync}^{2})$:
five are EXACT under $\mathcal R$ at machine precision
(spectral-dimension IR screening, structured occupancy
filtering, zero-mode cancellation, backreaction screening,
sync-channel un-cancellation) and two are sub-percent
System-$\mathcal R$-rational expressions (lattice vacuum
regularisation, Coleman--Gamow sequestering at $0.02\%$).
The remaining \emph{two layers are derived but not closed-form
System-$\mathcal R$ rationals}: L1$=(v_{\rm EW}/M_{\rm Pl})^{4}$
inherits the electroweak-scale derivation of Paper~1 (the full
HBR + $S^{4}$-bounce-action + two-loop construction) over the
Planck mass as a constant of nature; L5$=E_{\rm geom}/E_{\rm res}$
is derived from the carrier-lattice dynamics with a Symanzik
$1/N^{2}$ continuum extrapolation (bootstrap CI disclosed in the
\texttt{theory\_systematics} block). Neither L1 nor L5 is a fit
--- both are framework-derived --- but neither is expressible as
a rational in the System-$\mathcal R$ primitives. The honest
composite reading is therefore ``$9$ of $9$ derived, $0$ fits;
$7$ closed-form rational $+\,2$ derived-non-rational'', not
``$9$ of $9$ closed-form rational''.
...
theory systematic. An earlier candidate paired-degeneracy
correction does not enter the published chain, as structurally
...
\texttt{h\_sync\_promotion} and
\texttt{h197\_candidate\_review} entries in
...
and L4. The earlier split between the integer
...
full audit is at
\pyown{src/verify_cc_layer_audit_systemR.py}{verify_cc_layer_audit_systemR}; it recomputes
...
\pyscript{emergent-gr-closure-repro}{outputs/stage6f_full_tensor_norm_audit.json}{stage6f_full_tensor_norm_audit.json}) verifies
...
(equivalently $1 - (d{+}N_{\rm gen})\,\gamma\,\varepsilon^{2}_{\text{sync}}$
using $\varepsilon^{2}_{\text{sync}} = \gamma/2$), matching the
Planck~2018~\cite{Planck2018} best-fit $n_{s} = 0.9649 \pm 0.0042$
within $0.02\sigma$ and remaining inside the
end-of-2025 CMB+BAO combined posterior
$n_{s} = 0.9682 \pm 0.0032$~\cite{Balkenhol2025}. The
tensor-to-scalar ratio $r_{\text{pred}} = 0.029$ is a factor
$\sim 1.2$ below the most recent 95\% C.L.\ combined
Planck--SPT--ACT--BICEP/Keck--DESI bound
$r < 0.034$~\cite{Balkenhol2025} (and a factor $\sim 1.9$ below
the earlier BK18 stand-alone bound
$r < 0.056$~\cite{BICEPKeck2021}).
...
content-hashed in the bundled pre-registration record
\pyown{data/preregistration_2026_05_11.json}{preregistration_2026_05_11.json}
...
\pyown{src/build_preregistration.py}{build_preregistration} recomputes the hash
...
limit and the DESI~DR2 upper bound under $\Lambda$CDM;
decisive at CMB-S4 / Roman / Euclid Year-1-2.
...
The audit
(\pyown{src/verify_cosmic_horizon_info_capacity.py}{verify_cosmic_horizon_info_capacity},
output
\pyown{outputs/cosmic_horizon_info_capacity.json}{cosmic_horizon_info_capacity.json})
...
L.~Balkenhol \textit{et al.}, \emph{Inflation at the End of 2025:
Constraints on $r$ and $n_s$ Using the Latest CMB and BAO Data},
arXiv:2512.10613 (December 2025). Combined Planck+SPT+ACT+BICEP/Keck
analysis with DESI BAO, yielding $n_s = 0.9682 \pm 0.0032$ (CMB only)
shifting to $0.9728 \pm 0.0029$ with BAO, and the 95\% C.L. upper
bound $r < 0.034$.

\bibitem{Planck2018}
Planck Collaboration, N.~Aghanim \textit{et al.},
\emph{Planck 2018 results.~VI.~Cosmological parameters},
Astronomy~\&~Astrophysics \textbf{641}, A6 (2020); arXiv:1807.06209;
...
Physical Review D \textbf{112}, 083515 (2025); arXiv:2503.14738.
BAO measurement from $14$ million galaxies and quasars across
three years of operation; under flat-$\Lambda$CDM the DESI+CMB
combination yields the tightest current cosmological-parameter
constraints. The dynamical dark-energy $(w_0, w_a)$
parametrisation is preferred over $\Lambda$CDM at $3.1\sigma$,
representing the principal late-time DE tension addressed by the
framework's
$w_{\rm DE}\!=\!-1\!+\!\varepsilon_{\rm sync}^{4}/\gamma\!=\!-39/40$
closure (Section~\ref{sec:dm-de}).

\bibitem{WangMota2025}
D.~Wang and D.~Mota,
\emph{Did DESI DR2 truly reveal dynamical dark energy?},
European Physical Journal C (2025); arXiv:2504.15222.
Independent analysis arguing that the DESI-DR2 preference
for evolving $w(z)$ is not robust once tensions among the
CMB, BAO and SNe data sets are propagated through the
combined likelihood; the active counter-line to the
DESI~2024+2025 dynamical-DE claim.

\bibitem{Lieu2024}
R.~Lieu,
\emph{The binding of cosmological structures by massless
topological defects},
Monthly Notices of the Royal Astronomical Society
\textbf{531}, 1630 (2024); arXiv:2406.04355.
Demonstrates that a distribution of massless topological
defects produces flat stellar / galactic rotation curves and
isothermal-sphere-style lensing signatures \emph{without an
underlying particle component} -- the literature precedent
for the present identification of the halo signal with
$T_{\mu\nu}^{\rm DM\text{-}vortex}$ of the relational
vortex-defect network rather than with particulate DM.
...
\bibitem{DiValentino2025Tensions}
E.~Di Valentino \textit{et al.},
\emph{Cosmological tensions in the era of precision cosmology:
insights from Tensions in Cosmology 2025},
arXiv:2509.25288 (2025).
Community-white-paper-style review consolidating the
post-DESI-DR2 + JWST landscape; reports a Hubble-tension
discrepancy now exceeding $\sim\!6\sigma$ and surveys the
EDE, modified-gravity, and dark-sector remedies. Sets the
state-of-the-art empirical context for the present
framework's structural prediction
$H_{0}\!=\!100\,\alpha_{\xi}\,s_{\rm face}N_{\rm gen}\!=\!67.5$~km/s/Mpc,
which sits on the early-universe side of the persistent
late-time/early-time split.

\bibitem{Riess2024JWST}
A.~G.~Riess \textit{et al.},
``JWST Validates HST Distance Measurements: Selection of
Supernova Subsample Explains Differences in JWST Estimates of
Local $H_{0}$'',
Astrophysical Journal \textbf{977}, 120 (2024); arXiv:2408.11770.
SH0ES JWST cross-validation of the HST Cepheid distance ladder:
HST Cepheid distance linearity constrained to
$0.994\!\pm\!0.010$; three-method JWST result
$H_{0}\!=\!72.6\!\pm\!2.0$~km/s/Mpc on the distance-limited
$D\!\leq\!25$~Mpc sample, maintaining the $\sim\!73$~km/s/Mpc
SH0ES value with HST--JWST mutual consistency at $\sim\!1\sigma$.
Attributes prior CCHP-vs-SH0ES discrepancies to supernova-subsample
selection rather than Cepheid-calibration bias; the persistence
of the Cepheid-vs-TRGB/JAGB split therefore remains the principal
$H_{0}$-tension load-bearing observation under JWST validation.
