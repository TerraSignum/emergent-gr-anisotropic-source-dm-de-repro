...
which contributes nothing to any rank-correlation. The
...
an NFW-class halo carried by the residual energy density.}
...
\!\gamma/2\!=\!1/20$.} The matter-localised threshold
...
\paragraph{Resolution of the Hubble tension as a
...
Eq.~\eqref{eq:matter_local_classifier} against three matter-core
indicators (top-decile $t_{00}(a)$ from the per-node Galerkin
construction, top-decile $\Delta(a)$, and the per-node
topological winding number $|w(a)|\!>\!1/2$) on $26{,}704$
lattice nodes from the eleven $\psi$-bundled regimes on the
$\mathcal{P}_{5}/\mathcal{P}_{6}/\mathcal{P}_{8}$ ladder ($\mathcal{P}_{5}$ at $N\!=\!64,72,84,100,
128,200,256,300,512$ and $\mathcal{P}_{6}, \mathcal{P}_{8}$ at $N\!=\!128$,
including the post-flip regimes $N\!\geq\!200$ up to the
deep-post-flip endpoint $N\!=\!512$). The framework matter-
core indicators $\Delta(a), t_{00}(a)$ and $w(a)$ require the
local order parameter $\psi$ to be bundled, which restricts
the test corpus to these eleven regimes. Pooled metrics:
AUC$\,=\!0.843$ and matter-core enrichment ratio
$P(C_{N}|\theta_{\rm loc}\!>\!\theta_{\rm glob})/
P(C_{N}|\theta_{\rm loc}\!\leq\!\theta_{\rm glob})\!=\!10.46\!\times$
against $t_{00}(a)$. Per-regime AUC against $t_{00}(a)$ ranges
$0.80$--$0.89$ across all eleven regimes (slight downturn at the
deep post-flip endpoint $N\!=\!512$, AUC$\,=\!0.798$), with
...
pooled ratio $1.84\!\times$ against $t_{00}(a)$, since most
...
percentile resamples): AUC$\,=\!0.849$ $[0.838, 0.860]$, ratio
$11.31\!\times$ $[9.24, 14.19]$ against $t_{00}(a)$;
AUC$\,=\!0.616$ $[0.594, 0.637]$, ratio $1.75\!\times$
$[1.57, 1.97]$ against $\Delta(a)$. Both AUC and enrichment-
ratio confidence intervals exclude the null (AUC$\,=\!0.5$,
ratio$\,=\!1$) by a wide margin against $t_{00}(a)$ and
$\Delta(a)$; the winding indicator $|w(a)|$ has a confidence
interval that includes the null, reflecting the small available
sample (only the two regimes at $N\!=\!64,100$ bundle a winding
map; $n\!=\!656$).
(ii)~Leave-one-regime-out (LORO): the row-variance of $\Xi$ is
selected as the top per-node candidate by training AUC on the
remaining seven regimes in $8/8$ folds for both the $t_{00}(a)$
and $\Delta(a)$ targets, with held-out test AUC ranging $0.83$--
$0.90$ against $t_{00}(a)$ and $0.51$--$0.90$ against
$\Delta(a)$. The out-of-sample peak AUC reaches $0.904$ on the
$N\!=\!64$ regime, confirming that the per-node enrichment is
...
$t_{00}(a)$ is $z\!=\!37.9\sigma$ above the null mean
$0.498$, $p\!=\!0.001$ (permutation-test floor); observed AUC
against $\Delta(a)$ is $z\!=\!12.7\sigma$ above null,
...
($\Delta\!=\!0$ gives ratio $11.31\!\times$) and rises to a peak
$59.9\!\times$ at $\Delta\!\sim\!-11^{\circ}$, consistent with
...
$t_{00}(a)$ matter loading at AUC$\,=\!0.849$ pooled with
$0.83$--$0.90$ across all eight individual regimes and matter-
core enrichment $9.4$--$27\!\times$, exceeding the strict
$0.85$ promotion gate within five of eight regimes and at the
gate when pooled.
...
the $\mathcal{P}_{5}$ ladder ($N\!=\!100,128,200,256,300$) plus the
$\mathcal{P}_{6}, \mathcal{P}_{8}$ regimes at $N\!=\!128$; values above $1$
indicate matter-side nodes are more likely to lie in
$C_{N}$.}
...
\caption{Robustness audit of the local RG-window probe. Panel
A: bootstrap $95\%$ confidence intervals for AUC (left axis,
blue circles) and matter-core enrichment ratio (right axis,
orange squares) under the three matter-core indicators; both
quantities exclude the null (AUC$\,=\!0.5$, ratio$\,=\!1$).
Panel B: leave-one-regime-out test AUC for each held-out regime
on the $\mathcal{P}_{5}/\mathcal{P}_{6}/\mathcal{P}_{8}$ ladder; the row-variance of $\Xi$
remains the top candidate selected by training AUC on the
remaining seven regimes in $8/8$ folds for the $t_{00}(a)$ and
$\Delta(a)$ targets. Panel C: within-regime permutation null
distribution (1000 replicates) versus the observed AUC; the
observed AUC against $t_{00}(a)$ is $37.9\sigma$ above the null
mean. Panel D: matter-core enrichment ratio as a function of
...
prefactor of the V$_{us}\!=\!\gamma\sqrt{5}$ closure) with
the chirality-cosine half. The $109\!=\!108\!+\!1\!=\!12\,d\,N_{\rm gen}\!+\!1$
...
algebraic closure on the lattice $\rho_\Lambda$.
...
repository at \url{https://github.com/[anonymized]/emergent-gr-anisotropic-source-dm-de-repro}.
...
any reviewer can independently verify that the predictions
...
(\url{https://github.com/[anonymized]/emergent-gr-anisotropic-source-dm-de-repro}) bundles all
